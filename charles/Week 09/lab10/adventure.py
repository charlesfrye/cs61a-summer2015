...
name = 'Player' # Can replace this with your name. :)
...
def adv_parse(line):
    ...
    token = tokens.pop(0)
    if token == 'give':
        return (token, ' '.join(tokens[:-2]) ,tokens[-1])
    elif token == 'ask':
...
def adv_eval(exp):
    ...
    if is_person(exp):
        ...
        return give(adv_eval(exp[2]),adv_eval(exp[1]))
    elif exp[0] == 'ask':
        return ask(adv_eval(exp[1]),adv_eval(exp[2]))
    else:
...
class Place(object):
    ...
    def look(self):
        ...
        if not (self.people or self.things):
            result += 'Nothing!'
        result += '\n'.join([thing.name + ' - ' + 
            thing.description for thing in self.things])
